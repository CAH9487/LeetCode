...
// Runtime: 444 ms, faster than 10.44% of C online submissions for Longest Palindromic Substring.
// Memory Usage: 181.6 MB, less than 5.13% of C online submissions for Longest Palindromic Substring.
...
{
    int len = strlen(s);
    int nMax = 1, pos, max_right = -1;
    char *ss = (char *)malloc(((len << 1) + 1) * sizeof(char));
    int *p = (int *)malloc((len << 1) * sizeof(char));
